;;; cpr-disamb.el --- disambiguate ambiguous cites -*- lexical-binding: t; -*-
...
TYPE is either 'add-names or 'show-given-names."
  (let ((vv (cpr-itemdata-varvals itd)))
	...
	(let* ((cur-level (alist-get key (alist-get type vv)))
      ...
      (push `(,type . ((,key . 1))) (cpr-itemdata-varvals itd))))
  (setf (cpr-itemdata-rc-uptodate itd) nil))

(defun cpr-itd-add-name (itd style &optional _first-step)
...
no previous items to the left could be."
  (let* ((vars (cpr-itd-namevars itd style))
	 (cite (cpr-itd-plain-cite itd style))
	 (vv (cpr-itemdata-varvals itd))
	 (levels (alist-get 'add-names vv))
	 (remaining-vars (if levels (memq (caar levels) vars) vars))
	 (success nil))
    (while (and (not success) remaining-vars)
      (cpr-itd-inc-disamb-level (car remaining-vars) itd 'add-names)
      (if (string= cite (cpr-itd-plain-cite itd style))
	  (pop remaining-vars)
	(setq success t)))
    success))

(defun cpr-itd-add-given (itd style &optional first-step)
...
otherwise."
  (let* ((nids (cpr-itd-nameids itd style))
	 (cite (cpr-itd-plain-cite itd style))
	 (vv (cpr-itemdata-varvals itd))
	...
	(if (and current-level (>= current-level 2))
	    (pop remaining-nids)
	  (cpr-itd-inc-disamb-level current-nid itd 'show-given-names)
	  (unless (string= cite (cpr-itd-plain-cite itd style))
	    (setq success t)
...
(defun cpr-itd-addgiven-with-addname (itd style &optional first-step)
...
could be performed and t otherwise."
  (let* ((vv (cpr-itemdata-varvals itd))
	 ...
	 (success nil)
	 (remaining-names t))
    (if (and (not first-step) gn (= 1 (cdar gn)) (cpr-itd-add-given itd style))
	t
      (while (and (not success) remaining-names)
	(let ((nids (cpr-itd-nameids itd style)))
	  (if (cpr-itd-add-name itd style)
	      (let* ((new-nids (cpr-itd-nameids itd style))
		...
		(when first-step
		  (setf (alist-get new-nid
			...
			0))
		(when (cpr-itd-add-given itd style first-step)
		  (setq success t)))
	    (setq remaining-names nil))))
      success)))

(defun cpr-disamb--different-cites-p (itds style)
  "Whether some itemdata in ITDS have different cites with STYLE."
  (--any-p (not (string= (cpr-itd-plain-cite it style)
			 (cpr-itd-plain-cite (car itds) style)))
	   (cdr itds)))

(defun cpr-disamb--with-method (itds style disamb-fun)
...
successful and nil otherwise."
  (let ((orig-settings (copy-tree (cpr-disamb--settings itds)))
	success
	(first-step t))
    (while (and (not success)
		(--all-p (funcall disamb-fun it style first-step) itds))
      (setq first-step nil)
      (when (cpr-disamb--different-cites-p itds style)
	(setq success t)))
    (unless success
      (cpr-disamb--restore-settings itds orig-settings))
    success))

(defun cpr-disamb--settings (itds)
  "Return a list with the disamb settings of ITDS."
  (--map (cons (cpr-itd-getvar it 'add-names)
	       (cpr-itd-getvar it 'show-given-names))
	 itds))

(defun cpr-disamb--restore-settings (itds settings)
  "Restore the disamb settings of ITDS from SETTINGS.
SETTINGS should have the structure produced by cpr--disamb-settings."
  (cl-loop for itd in itds
	   for (add-names-val . show-given-val) in settings do
	   (cpr-itd-setvar itd 'add-names add-names-val)
	   (cpr-itd-setvar itd 'show-given-names show-given-val)))

(defun cpr-disamb--num-to-yearsuffix (n)
...
(defun cpr-disamb--add-yearsuffix (itds _style)
  "Disambiguate itemdata in ITDS by adding year suffices.
Return t (for the sake of consistency with other disamb methods)."
  (--each-indexed (--sort (< (string-to-number (cpr-itd-getvar it 'citation-number))
			     (string-to-number (cpr-itd-getvar other 'citation-number)))
			  itds)
    (cpr-itd-setvar it 'year-suffix (cpr-disamb--num-to-yearsuffix it-index))
    (setf (cpr-itemdata-rc-uptodate it) nil))
  t)

(defun cpr-disamb--set-fields (itds)
  "Disambiguate ITDS by setting their disambiguate fields."
  (--each itds
    (cpr-itd-setvar it 'disambiguate t)))

(defun cpr-disamb-amb-itds (itds style name given yearsuff)
...
otherwise."
  (or (and name (cpr-disamb--with-method itds style 'cpr-itd-add-name))
      (and given (cpr-disamb--with-method itds style 'cpr-itd-add-given))
      (and name given
	   (cpr-disamb--with-method itds style 'cpr-itd-addgiven-with-addname))
      (progn
	(cpr-disamb--set-fields itds)
	(cpr-disamb--different-cites-p itds style))
      (and yearsuff
	   (cpr-disamb--add-yearsuffix itds style)
	   (cpr-disamb--different-cites-p itds style))))

(defun cpr-amb-itds (itds style)
...
The returned value is a (possibly empty) list of lists."
  (let* ((sorted (-sort (lambda (x y)
			  (string< (cpr-itd-plain-cite x style)
				   (cpr-itd-plain-cite y style)))
			itds))
	 (result nil)
	...
	(if (string= (cpr-itd-plain-cite act style)
		     (cpr-itd-plain-cite next style))
	    (progn
	      (push next act-list)
	      (setq ambig t))
...
(defun cpr-disamb-itds (itds style name given yearsuff)
  "Disambiguate itemdatas in list ITDS for STYLE.
NAME, GIVEN and YEARSUFF are generalized booleans specifying
whether or not the add-name, show-given or add-year-suffix
disambiguation methods should be used."
  (let ((amb-itds (cpr-amb-itds itds style)))
    (while amb-itds
      (let ((act-set (pop amb-itds)))
	(cpr-disamb-amb-itds act-set style name given yearsuff)
	(when (cpr-disamb--different-cites-p act-set style)
	  (when-let (new-ambs (cpr-amb-itds act-set style))
	    (setq amb-itds (nconc new-ambs amb-itds))))))))

(provide 'cpr-disamb)

;;; cpr-disamb.el ends here
